...
const { MSSQLTestConnection } = require("re-mssql");
...
const encrypt = process.argv[21];
const trustServerCertificate = process.argv[22];
...
(async () => {
  ...
  const ssl =
    sslEnabled === "true"
      ...
      : new NoSsl();
  const reverseEngineering = new MSSQLTestConnection(
    host,
    port,
    user,
    encrypt === "true",
