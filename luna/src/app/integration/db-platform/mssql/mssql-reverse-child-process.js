...
const { MSSQLReverseEngineering } = require("re-mssql");
...
const encrypt = process.argv[28];
const trustServerCertificate = process.argv[29];
...
(async () => {
  ...
  const ssl =
    sslEnabled === "true"
      ...
      : new NoSsl();
  const reverseEngineering = new MSSQLReverseEngineering(
    connectionId,
    connectionName,
    host,
    port,
