...
import { hasLicense } from "./features/features";
import watermarkGalaxy from "../assets/watermark-galaxy.svg";
import watermarkLuna from "../assets/watermark-luna.svg";
...
const UIHelpers = {
  ...
  toggleNestedVisibility(subId) {
    var elements = document.querySelectorAll("." + subId);
    elements.forEach((element) => {
      element.classList.toggle("im-display-inline-block");
      element.classList.toggle("im-display-none");
    });

    var iconElements = document.querySelectorAll(".icon_" + subId);
  ...
  getWatermarkByProduct(productName) {
    switch (productName) {
      ...
      case "LunaModeler":
        return watermarkLuna;
      case "GalaxyModeler":
        return watermarkGalaxy;
      default:
        return watermarkMoon;
    }
  },

  renderWatermark(profile, productName) {
    return (
      !hasLicense(profile) && (
        <image
          className="watermark"
