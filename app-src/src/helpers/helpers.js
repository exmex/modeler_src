...
const Helpers = {
  ...
  formatZero(value) {
    if (value === "0" || value  === 0) {
      return (<span className="im-text-secondary">-</span>);
    } else return value;
  },
  ...
  formatBreadcrumb(value) {
    ...
    if(lastDot !== -1){
    const prefixes = value.substr(0, lastDot);
    const name = value.substr(lastDot, _.size(value));
      return (<><span className="im-text-opacity-50">{prefixes}</span><span>{name}</span></>);
    }
    else {
  ...
  prepareListForDeletion(table, list, allTables) {
    ...
    _.map(table.cols, (col) => {
      if (
        allTables[col.datatype] && allTables[col.datatype].embeddable === true
      ) {
