...
import { ClassOtherObjectMSSQL } from "../platforms/mssql/class_other_object_mssql";
...
import { ClassTableMSSQL } from "../platforms/mssql/class_table_mssql";
...
import MSSQLHelpers from "../platforms/mssql/helpers_mssql";
...
import { updateMSSQLTargetColumn } from "../actions/platforms/mssql/mssql";
...
class DiagramTableList extends Component {
  ...
  async updateCollapsedState(itemId) {
    const collapsedState = this.props.activeDiagramObject.diagramItems[itemId]
      ?.collapsed
      ? this.props.activeDiagramObject.diagramItems[itemId].collapsed
      : false;
    ...
    this.props.setForcedRender({ domToModel: true });
  }
  ...
  showSchemaContainer(object) {
    if (
      ...
      if (this.props.type === ModelTypes.PG) {
        ...
        return Helpers.getSchemaContainer(object.mssql.schema);
  ...
  async handleKeyDown(event) {
    ...
    const col = _.find(this.props.allTables[this.props.match.params.id]?.cols, [
      "id",
      this.props.match.params.cid
    ]);
    const subId = "im-" + col?.datatype;

    if (
      (event.ctrlKey && event.key === "ArrowRight") ||
      (event.metaKey && event.key === "ArrowRight")
    ) {
      await UIHelpers.toggleNestedVisibility(subId);
      await this.updateCollapsedState(col?.datatype);
  ...
  async navigateLeft() {
    if (this.props.match.params.cid) {
      const parentColId =
        this.props.catalogTables[this.props.match.params.id]?.parentColId;
      const parentTableId =
        this.props.catalogColumns[parentColId]?.belongsToTableId;
      if (parentColId && parentTableId) {
        this.props.history.push(
          "/model/" +
            this.props.match.params.mid +
            "/diagram/" +
            this.props.match.params.did +
            (this.props.allTables[parentTableId].embeddable
              ? "/emb/"
              : "/item/") +
            parentTableId +
            "/col/" +
  ...
  async navigateRight() {
    if (this.props.match.params.cid) {
      const col = _.find(
        this.props.allTables[this.props.match.params.id].cols,
        ["id", this.props.match.params.cid]
      );
      if (col && this.props.allTables[col.datatype]) {
        if (
          this.props.activeDiagramObject.diagramItems[col.datatype]
            ?.collapsed !== true
        ) {
          const firstNestedCol = this.props.allTables[col.datatype].cols[0];
          if (firstNestedCol) {
            this.props.history.push(
              "/model/" +
                this.props.match.params.mid +
  ...
  async navigateUp() {
    if (this.props.match.params.cid) {
      ...
      if (
        ...
        this.props.allTables[this.props.match.params.id].cols[colPosition - 1]
          .name !== "FakeIdForInternalUse" &&
        this.props.allTables[this.props.match.params.id].cols[colPosition - 1]
          .isHidden !== true
      ) {
        this.props.history.push(
          "/model/" +
            ...
            "/diagram/" +
            this.props.match.params.did +
            (this.props.allTables[this.props.match.params.id].embeddable
              ? "/emb/"
              : "/item/") +
  ...
  isMongooseSchema(table) {
    return table.embeddable === false;
  }
  ...
  async updateHeight() {
    if (
      ...
      let rootTableId =
        this.props.catalogColumns[this.props.match.params.cid]?.rootTableId;
      if (rootTableId) {
        let newHeight = Math.round(
          this.calcAllNested(this.props.allTables[rootTableId]) *
            (18 * this.props.zoom) +
            28 * this.props.zoom
        );
        this.props.updateDiagramItemProperty(
          this.props.activeDiagramObject.id,
          rootTableId,
          newHeight,
          "gHeight"
  ...
  getTableCols(mainTableId, table, prefix, showNested) {
    try {
      var i = 0;
      return _.map(table.cols, (col) => {
        i++;
        var colName = showNested
          ? col.name
          : Helpers.shortenString(col.name, false, 20);
        if (col.isHidden === true) {
          ...
          if (this.props.currentDisplayMode === "description") {
            ...
            if (embeddedTable !== undefined) {
              ...
              if (embeddedTable) {
                if (
                  this.props.allTables[col.datatype] !== undefined &&
                  !this.isMongooseSchema(this.props.allTables[col.datatype])
                ) {
                  newPrefix = prefix + col.name + ".";
                  newPrefix = "";
                }
                if (this.props.embeddedInParentsIsDisplayed !== false) {
                  const subId = "im-" + embeddedTable.id;
                  const selectedEmb =
                    this.props.match.params.id === table.id &&
                    this.props.match.params.cid === col.id
                      ? ` ${subId}  emb-selected-box`
                      : ` ${subId} `;

                  const selectedEmbBox =
                    this.props.match.params.id === embeddedTable.id
                      ? ` ${subId}  emb-selected`
                      : ` ${subId} `;

                  expanderGraphics = (
                    <div
                      className="im-expander im-pointer"
                      onClick={async (e) => {
                        e.persist();
                        if (
                          ...
                          this.props.updateUiProperty(
                            1,
                            "activeColumnDetailTab"
                          );
                        }

                        UIHelpers.toggleNestedVisibility(subId);
                        await this.updateCollapsedState(embeddedTable.id);
                        e.stopPropagation();
                        e.preventDefault();
                      }}
                    >
                      <i
                        className={`im-icon-14 ${
                          this.props.activeDiagramObject.diagramItems[
                            embeddedTable.id
                          ]?.collapsed !== true
                            ? " im-icon-Visibility16 "
                            : " im-icon-Hidden16 "
                        }  icon_${subId}`}
                      />
                    </div>
                  ...
                  embeddedGraphics = (
                    <>
                      ...
                      >
                        <div>
                          {this.shouldShowPerseidERDNestedTables(
                            embeddedTable
                          ) &&
                            this.getTableCols(
                              mainTableId,
                      ...
                      <div
                        ...
                        className={
                          JsonSchemaHelpers.isPerseidModelType(this.props.type)
                            ...
                            : `im-embedded ${selectedEmb} ${selectedEmbBox}`
                        }
                        style={{ gridColumn: "span 4" }}
                      >
                        <div
                        ...
                        >
                          <div
                            className="dNestedLink"
                            style={{ borderColor: nestedLinkColor }}
                          ></div>
                          <div
                            className="dGraphicsNestedHeader"
                            style={{ background: nestedHeaderBg }}
                          >
                            <div className="dGraphicsLeftTop"></div>

                            <div className="dTableName">
                              {col.name}{" "}
                              {col.list && (
                                <span className="im-col-list">[</span>
                              )}
                              <span className="im-col-object-left">
                                &#123; &#125;
                              </span>
                              {col.list && (
                                <span className="im-col-list">]</span>
                              )}
                              {col.nn && <span className="im-mini"> NN</span>}
                            </div>

                            <div className="dGraphicsRightTop"></div>
                          </div>
                          {this.shouldShowPerseidERDNestedTables(
                            embeddedTable
                          ) &&
            ...
            if (
              (this.props.type === "PG" || this.props.type === "MSSQL") &&
            ...
            if (this.props.type === "GRAPHQL") {
              ...
              colNameDisplayed = (
                <>
                  {Helpers.formatBreadcrumb(prefix + colName)}
                  {listGraphicsLeft}
                  {objectGraphicsLeft}
                  {objectGraphicsRight}
                  {listGraphicsRight}
          ...
          return (
            <div
              id={col.id}
              key={col.id}
              className={`${dRow} im-t-r im-t-r-${this.props.type}`}
            >
              <div className="dCol im-t-c">
              ...
              {this.props.type === ModelTypes.MONGODB ||
              this.props.type === ModelTypes.MONGOOSE ? (
                <div className="dCol im-t-c im-mini">{expanderGraphics}</div>
              ) : (
                ""
              )}
              {embeddedGraphics}
  ...
  async addSingleTable(pageX, pageY) {
      ...
      newKeys = [
        {
          id: uuidv4(),
          name: this.props.nameAutoGeneration.keys
            ? MSSQLHelpers.makeUniquePKName(
                defaultName,
                MSSQLHelpers.getAllKeysByNames(this.props.allTables)
              )
            : "Primary key",
          isPk: true,
    ...
    if (this.props.type === "MSSQL") {
      let defaultValues = {
        def_coltopk: this.props.def_coltopk,
        schema: this.props.mssql.schema
      };

      t = new ClassTableMSSQL(
        newTableId,
        defaultName,
        newCols,
        newKeys,
  ...
  async addSingleTextNote(pageX, pageY) {
  ...
  async addSingleOtherObject(pageX, pageY, otherObjectType) {
    ...
    try {
      ...
      } else if (this.props.type === "MSSQL") {
        t = new ClassOtherObjectMSSQL(
          uuidv4(),
          ...
          {
            schema: this.props.mssql.schema,
            ...(otherObjectType === "Sequence"
              ? { sequence: { start: "1", increment: "1", isCycling: true } }
              : {}),
            ...(otherObjectType === "UserDefinedType" ? { udt: {} } : {})
          }
        );
      } else {
        t = new ClassOtherObject(
          uuidv4(),
          otherObjectName, // this is name
          otherObjectType
        );
  ...
  handleDiagramAreaClick(e) {
    ...
    if (
      ...
      this.addSingleTextNote(e.pageX, e.pageY);
  ...
  xResize(e) {
    ...
    if (
      ...
      minHeight = Math.round(
        this.calcAllNested(this.props.tables[this.props.match.params.id]) *
          (18 * this.props.zoom) +
          28 * this.props.zoom
      );
    }
    ...
    let newHeight = Math.round(
      Math.max(e.clientY - elbox.y + body.scrollTop, minHeight) /
        this.props.zoom
    );
  ...
  renderSingleRelation(relation, path) {
    ...
    if (this.props.activeDiagramObject) {
      ...
      if (
        ...
        if (this.props.settings.theme === "im-dark") {
          textPathColor = "#333";
        } else if (this.props.settings.theme === "im-print") {
          textPathColor = "#fff";
        } else {
          textPathColor = "#eee";
        }
  ...
  calcAllNested(child) {
    ...
    if (this.props.embeddedInParentsIsDisplayed === true) {
      if (
        this.props.activeDiagramObject.diagramItems[child.id]?.collapsed !==
        true
      ) {
  ...
  getNoteColor(objColor) {
    let toReturn = objColor;
    if (objColor === "transparent") {
      if (this.props.settings.theme === "im-dark") {
        toReturn = "#ffffff";
      } else {
        toReturn = "#000000";
  ...
  renderNoteContent(obj) {
    ...
    const noteColor = this.getNoteColor(obj.color);
    return (
      <div
        className={className}
        style={{
  ...
  createNewColumn(
    ...
    if (colToAdd !== undefined) {
      ...
      updateMSSQLTargetColumn(this.props.type, colToAdd);
      updateGraphQLTargetColumn(
        this.props.type,
        newRelationObject,
        colToAdd,
  ...
  async createRelation(relid) {
    ...
    if (
      ...
      newRelationObject.name = MSSQLHelpers.makeRelationName(
        newRelationObject,
        { [targetTable.id]: targetTable },
        { ...this.props.relations, [newRelationObject.id]: newRelationObject }
      );
        ...
        <div
          className={
            isMoon(this.props.profile)
              ? "dGraphicsHeader dGraphicsHeaderMoon"
              : "dGraphicsHeader"
          }
  ...
  getTableContent(table, displayMode) {
    ...
    switch (displayMode) {
      ...
      default:
        return (
          <div className="dCols" id={"dc" + table.id}>
            {this.getTableCols(table.id, table, "", true)}
          </div>
        );
    }
  }
    ...
    return (
      ...
      >
        ...
        {this.renderObjects(
          ...
          isMoon(this.props.profile) ? `t tMoon ` : `t `
        )}
        {this.renderObjects(
          this.props.notes,
          this.props.match.params.nid,
