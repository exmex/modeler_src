...
import { createModel, setDisplayMode, showDiagram } from "../../actions/model";
...
import { setForcedRender, toggleNewModelModal } from "../../actions/ui";
...
import { ClassModelMSSQL } from "../../platforms/mssql/class_model_mssql";
...
class ModalNewModel extends Component {
  ...
  async createNewModel() {
    ...
    if (this.state.type === ModelTypes.MSSQL) {
      defaultValues = {
        def_coltopk: true
      };

      newModel = new ClassModelMSSQL(
        uuidv4(),
        diagram.id,
        this.state.name,
        this.state.desc,
    ...
    this.props.toggleNewModelModal();
    ipcRenderer?.send("modelSettings:save", filterModelSettings(newModel));
  }
