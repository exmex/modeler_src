...
class ConnectionsUpdate extends Component {
  constructor(props) {
    super(props);
    this.state = {
      searchTerm: ""
  ...
  renderConnections() {
    return (
      <Fragment>
        <div className="im-header-1 im-align-center">
          {_.upperFirst(this.props.localization.L_MODEL)} update
        </div>
        {this.renderSearchInput()}
        <ConnectionsList searchTerm={this.state.searchTerm} update={true} />
        <div className="im-align-center im-padding-md" />
      </Fragment>
    );
  }
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ConnectionsUpdate)
);
