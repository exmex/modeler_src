...
class OtherObjectCustomCode extends Component {
  async handleTextChange(propName, event) {
    const value = event.target.value;
    await this.props.startTransaction(
      getHistoryContext(this.props.history, this.props.match),
      UndoRedoDef.OTHER_OBJECT_CUSTOM_CODE__UPDATE_OTHER_OBJECT_PROPERTY
    );
    try {
  ...
  async handleCheckboxChange(propName, event) {
    ...
    await this.props.startTransaction(
      getHistoryContext(this.props.history, this.props.match),
      UndoRedoDef.OTHER_OBJECT_CUSTOM_CODE__UPDATE_OTHER_OBJECT_PROPERTY_BOOLEAN
    );
    try {
      await this.props.updateOtherObjectProperty(
        this.props.match.params.oid,
  ...
  render() {
    if (!this.props.match.params.oid || this.props.match.params.oid === null) {
      return null;
    }

    if (this.props.otherObjects[this.props.match.params.oid] === undefined) {
      return null;
    }
    ...
    return (
      <div className="im-properties-code">
        <DebounceInput
          ...
          value={Helpers.gv(
            this.props.otherObjects[this.props.match.params.oid].customCode
          )}
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(OtherObjectCustomCode)
);
