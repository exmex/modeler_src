...
export const ACTION_PROVIDE_MODEL = "action:provideModel";
export const ACTION_REOPEN_MODEL = "action:reopenModel";
export const ACTION_OPEN_FROM_URL = "action:openFromUrl";
export const ACTION_CONVERT_AND_SAVE_TEMP = "action:convertandSaveTemp";
...
export const TOGGLE_DIFF_HTML_REPORT_MODAL = "toggle_diff_html_report_modal";
...
export const TOGGLE_REVERSE_STATS_MODAL = "toggle_reverse_stats_modal";
...
export const CLEAR_BACKUP_MODEL_DIALOG = "clear_backup_model_dialog";
export const SET_BACKUP_MODEL_DIALOG = "set_backup_model_dialog";
...
export function toggleReverseStatsModal() {
  return {
    type: TOGGLE_REVERSE_STATS_MODAL
  };
}
...
export function toggleDiffHTMLReportModal() {
  return {
    type: TOGGLE_DIFF_HTML_REPORT_MODAL
  };
}
...
export function executeUnsavedChangesModalAction(
  ...
  return async (dispatch) => {
    switch (postponedAction.name) {
      ...
      case ACTION_CONVERT_AND_SAVE_TEMP: {
        await dispatch(
          convertAndSaveTemp(
            historyContext,
            ipcContext,
...
export function onToggleVisibilityClick(historyContext) {
  return async (dispatch, getState) => {
    const state = getState();
    const modified = [];
    if (_.size(state.selections) > 1) {
      await dispatch(
        startTransaction(historyContext, UndoRedoDef.UI__TOGGLE_OBJECTS_VISIBLE)
      );
      await Promise.all(
      ...
      if (historyContext.state.tableId) {
        modified.push(historyContext.state.tableId);
        await dispatch(
          updateTableProperty(
            historyContext.state.tableId,
            !state.tables[historyContext.state.tableId].visible,
            "visible"
          )
        );
      }
      if (historyContext.state.noteId) {
        modified.push(historyContext.state.noteId);
        await dispatch(
          updateNoteProperty(
            historyContext.state.noteId,
            !state.notes[historyContext.state.noteId].visible,
            "visible"
          )
        );
      }
      if (historyContext.state.otherObjectId) {
        modified.push(historyContext.state.otherObjectId);
        await dispatch(
          updateOtherObjectProperty(
            historyContext.state.otherObjectId,
            !state.otherObjects[historyContext.state.otherObjectId].visible,
            "visible"
          )
        );
      }
