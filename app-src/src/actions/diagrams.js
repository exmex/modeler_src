...
const CONTAINER_PG_MSSQL = "Schemas";
...
export function isToggleAddDiagramsByContainersActionVisible(type, profile) {
  return (
    (type === ModelTypes.MARIADB ||
      ...
      type === ModelTypes.MSSQL ||
      type === ModelTypes.PG) &&
    isFeatureAvailable(profile.availableFeatures, Features.MULTIDIAGRAMS)
  );
}
...
export function getDiagramItemElement(id) {
...
export function mergeChangesOriginal(r, i) {
  return {
    ...r,
...
function itemById(state, id) {
  return state.diagrams[state.model.activeDiagram].diagramItems[id];
}

function isMainDiagram(state) {
  return (
    state.model.activeDiagram &&
    state.diagrams[state.model.activeDiagram] &&
    state.diagrams[state.model.activeDiagram].main
  );
}

function notOnDiagramItemsReferenced(
  ...
  var objList = {};
  _.map(state.relations, (rel) => {
    if (addReferencedDiagramItems === "children") {
      if (objectsToIterate[rel.parent]) {
        objList = { ...objList, [rel.child]: { objectId: rel.child } };
      }
    } else if (addReferencedDiagramItems === "parents") {
      if (objectsToIterate[rel.child]) {
        objList = { ...objList, [rel.parent]: { objectId: rel.parent } };
      }
    } else {
      if (objectsToIterate[rel.parent]) {
        objList = { ...objList, [rel.child]: { objectId: rel.child } };
  ...
  return _.map(
    _.filter(
      objList,
      ({ objectId }) =>
        (!isMainDiagram(state) && !itemById(state, objectId)) ||
        (isMainDiagram(state) && getObjectById(getState, objectId)
          ? !getObjectById(getState, objectId).visible
          : false)
    ),
    (item) => ({
      item: getObjectById(getState, item.objectId),
      objectType: getObjectById(getState, item.objectId).objectType
    })
  );
}

export function addReferencedDiagramItems(
  ...
  return async (dispatch, getState) => {
    ...
    const itemsToAdd = notOnDiagramItemsReferenced(
      referencedParentChildOrAll,
      objectsToIterate,
      state,
      getState
    ...
    const mainDiagram = _.find(state.diagrams, ["main", true]);
    if (itemsToAdd) {
      for (const index of itemsToAdd.keys()) {
        ...
        const di = createDiagramItem(
          item,
          objectType,
          mainDiagram.diagramItems[item.id].x || 100,
          mainDiagram.diagramItems[item.id].y || 100 * (index + 1)
        );
        await dispatch(addDiagramItems(state.model.activeDiagram, [di]));
      }
    }
  };
}
