...
import { getCurrentHistoryTransaction, getModelState } from "./undoredo";
import {
  removeFromOrder,
...
export function getDefaultDataType(projectType, isEmbedded) {
  switch (projectType) {
    ...
    if (_.size(getState().selections) > 0) {
      ...
      for (const selection of selections) {
        if (selection.objectType === "table") {
          await dispatch(deleteComplex(historyContext, selection.objectId));
...
export function deleteTable(historyContext, id) {
  return async (dispatch, getState) => {
    ...
    await dispatch({
      type: DELETE_TABLE,
      payload: { id, profile: getState().profile }
    });
...
export function removeChildTable({ childTableId }) {
  return async (dispatch, getState) => {
    await dispatch({
      ...
      payload: { childTableId, profile: getState().profile }
    });
  };
}
...
export function deleteColumnById(tableId, columnId) {
  return async (dispatch, getState) => {
    await dispatch({
      ...
      payload: { tableId, columnId, profile: getState().profile }
    });
  };
}
...
export function deleteColumn(tableId, position, columnId) {
  return async (dispatch, getState) => {
    await dispatch({
      type: DELETE_COLUMN,
      payload: { tableId, position, columnId, profile: getState().profile }
    });
  };
}
...
export function fetchTableAndCatalog(table) {
  return async (dispatch, getState) => {
    await dispatch({
      ...
      payload: { table, tables: getState().tables, profile: getState().profile }
    });
  };
}
...
export function setDataTypeComplex(
  historyContext,
  colId,
  tableId,
  newDataTypeName,
  ...
  return async (dispatch, getState) => {
    ...
    const tableToDelete = getDatatypeTableByDatatypeColId(
      getState().tables,
      getState().catalogColumns.colToTable,
      colId
    );

    let isNotMongooseSchema = tableToDelete?.embeddable;

    if (
      tableToDelete &&
      newDataTypeName !== hierarchyTypeName &&
      isNotMongooseSchema
    ) {
      let listOfObjectToDelete = Helpers.prepareListForDeletion(
        getState().tables[tableToDelete.id],
        [],
        getState().tables
      );

      for (const l of listOfObjectToDelete) {
        if (getState().tables[l]) {
          await dispatch(deleteTable(historyContext, l));
        }
      }
    }

    if (newDataTypeName === hierarchyTypeName) {
      newColDataType = uuidv4();
      let newInternalDataTypeTable = new ClassTable(
        newColDataType,
        hierarchyTypeName,
        [],
        null,
...
export function deleteComplex(historyContext, tableId) {
  return async (dispatch, getState) => {
    const tableToDelete = getState().tables[tableId];
    if (tableToDelete) {
      let listOfObjectToDelete = Helpers.prepareListForDeletion(
        getState().tables[tableId],
        [],
        getState().tables
      );
      for (const l of listOfObjectToDelete) {
...
export function addToRootJsonSchema(
  ...
  return async (dispatch, getState) => {
    ...
    navigateByObjectType(
      ...
      finalCol,
      getState().tables[
        getState().catalogColumns.colToTable[finalColId].belongsToTableId
      ]
    );
    TreeDiagramHelpers.navigateByColId(finalColId, getState().ui.zoom);
  };
}

function getDatatypeTableByDatatypeColId(tables, colToTable, colId) {
...
export function addDatatypeColTable(
  historyContext,
  table,
  newPropertyName,
  columnType
...
export function createIntermediaryKeyTable(
  ...
  return async (dispatch, getState) => {
    ...
    navigateByObjectType(
      ...
      intermediaryCol,
      getState().tables[
        getState().catalogColumns.colToTable[intermediaryCol.id]
          .belongsToTableId
      ]
    );

    TreeDiagramHelpers.navigateByColId(intermediaryCol.id, getState().ui.zoom);
  };
}
