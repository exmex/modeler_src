...
const MongoDbHelpers = {
  ...
  convertMongoDBDataTypesToMongoose(dataTypeName) {
    if (_.includes(this.getMongoDBDataTypes(), dataTypeName)) {
      switch (dataTypeName) {
        case "double":
        case "int":
          return "Number";
  ...
  makeDatatypesHierarchy(datatypeId, allTables) {
    let typeName = this.getMongoDBHierarchyType();
    let value = allTables[datatypeId] ? datatypeId : typeName;
    return (
