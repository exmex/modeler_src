...
import { getMongoDbDefineStatement } from "./generator_mongodb";
...
class ModelMongoDb extends Component {
    ...
    _.map(collections, (table) => {
      if (
        ...
        toReturn = {
          ...
          [table.id]: {
            name: table.name,
            script: this.getMongoDb(table, false)
          }
        };
      }
    });
  ...
  getMongoDb(table, beautifyCode) {
    var code;
    if (beautifyCode === true) {
      code = js_beautify(
        getMongoDbDefineStatement(table, "'", this.props.tables, "", {
          previewObject: false
        }),
        {
          indent_size: 2,
      ...
      code = getMongoDbDefineStatement(table, "'", this.props.tables, "", {
        previewObject: false
      });
  ...
  getMongoDbScript() {
    ...
    return _.map(collections, (table) => {
      if (
        ...
        return this.getMongoDb(table, true);
      }
    });
  }
    ...
    return (
      <div className="modal-toolbar">
        <button
          ...
          onClick={() =>
            this.copyToClipboard(this.getScript(this.getMongodbArray()))
          }
        >
          Copy to clipboard
        </button>
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ModelMongoDb)
);
