...
/* tables */
export const getPgTableDetailForReport = (
  table,
  tables,
  relations,
  lines,
  ...
  if (table !== null) {
    ...
    if (table.objectType === "composite") {
      code += getCompositeProps(table, tables);
    }
    ...
    if (table.objectType && table.objectType === "table") {
      ...
      code += getCompositeColumnInfoForReport(table, tables, otherObjects);
    ...
    code += getPgTableIndexesForReport(table.indexes, table.cols);
    if (table.objectType && table.objectType === "table") {
      code += getPgTableKeysForReport(table.keys, table.cols);
    }
    code += getPgTableRelationsForReport(table.relations, tables, relations);
...
export const getPgRelationDetailForReport = (relation, tables) => {
  if (relation !== null) {
    ...
    code += getPgRelationKeysForReport(
      relation,
...
const getPgRelationKeysForReport = (relation, parentTable, childTable) => {
  var code = "";
  code += ReportHelpers.getCssWrapperStart("mm-subsection");
...
const getPgTableKeysForReport = (tableKeys, tableCols) => {
...
const getPgTableIndexesForReport = (tableIndexes, tableCols) => {
  ...
  for (var ix of tableIndexes) {
    ...
    code += ReportHelpers.getCaptionAndValueWrapper("Using", ix.algorithm); // ???
    code += ReportHelpers.getCaptionAndValueWrapper(
      "With / storage parameters",
      ix.pg?.storageparameters
    );
    code += ReportHelpers.getCaptionAndValueWrapper(
      "Tablespace",
      ix.pg?.tablespace
    ...
    code += ReportHelpers.getCaptionAndValueWrapper("Description", ix.pg?.desc);
    code += ReportHelpers.getCssWrapperEnd("mm-d-hor");
  }
...
export const getPgDomain = (otherObject, colspan) => {
  var code = "";
  var otherOptions = [];
  otherOptions = [
    ...otherOptions,
    ReportHelpers.getCaptionAndValueWrapper(
      "Datatype",
      otherObject.pg?.domain?.datatype
    )
  ];
  otherOptions = [
    ...otherOptions,
    ReportHelpers.getCaptionAndValueWrapper(
      "Parameters",
      otherObject.pg?.domain?.datatype_param
    )
  ];
  otherOptions = [
    ...otherOptions,
    ReportHelpers.getCaptionAndValueWrapper(
      "Collate",
      otherObject.pg?.domain?.collation
    )
  ];
  otherOptions = [
    ...otherOptions,
  ...
  otherOptions = [
    ...
    ReportHelpers.getCaptionAndValueWrapper(
      "Not null",
      otherObject.pg?.domain?.not_null
    )
  ];
...
const getPgTableRelationsForReport = (tableRelations, tables, relations) => {
...
const getCompositeColumnInfoForReport = (table, tables, otherObjects) => {
  if (table.cols.length === 0) {
    return "";
  }
  var code = "";
  for (var col of table.cols) {
    ...
    code += ReportHelpers.getValueDivWrapper(col.param);
    code += ReportHelpers.getValueDivWrapper(col.list);
    code += getCompositeColProps(col, 4);
  }
...
const getColDatatype = (datatype, tables, otherObjects) => {
  ...
  if (tbl) {
    ...
    } else if (tbl.objectType === "composite") {
      return `<a href='index.html?page=Composites&item=itm-${tbl.id}'>${tbl.name}</a>`;
    } else {
...
const getTableProps = (table, tables) => {
  ...
  otherOptions = [
    ...otherOptions,
    ReportHelpers.getCaptionAndValueWrapper("Schema", table.pg?.schema)
  ];
  otherOptions = [
    ...otherOptions,
    ReportHelpers.getCaptionAndValueWrapper("Tablespace", table.pg?.tablespace)
  ];
  otherOptions = [
    ...otherOptions,
    ReportHelpers.getCaptionAndValueWrapper(
      "Storage parameters",
      table.pg?.storageParameters
    )
  ];
  otherOptions = [
    ...otherOptions,
    ReportHelpers.getCaptionAndValueWrapper("Partitions", table.pg?.partition)
  ];
  otherOptions = [
    ...otherOptions,
    ReportHelpers.getCaptionAndValueWrapper("Inherits", table.pg?.inherits)
  ];
  otherOptions = [
    ...otherOptions,
    ReportHelpers.getCaptionAndValueWrapper(
      "Row level security",
      table.pg?.rowsecurity
    )
  ];
  otherOptions = [
    ...otherOptions,
...
const getCompositeProps = (table, tables) => {
  var code = "";
  var otherOptions = [];
  otherOptions = [
    ...otherOptions,
...
const getCompositeColProps = (col, colspan) => {
  var code = "";
  var otherOptions = [];
  otherOptions = [
    ...otherOptions,
...
const getColProps = (col, colspan) => {
  ...
  otherOptions = [
    ...
    ReportHelpers.getCaptionAndValueWrapper(
      "Generated identity",
      col.pg?.generatedIdentity !== "no" ? col.pg?.generatedIdentity : ""
    )
  ];
  otherOptions = [
    ...otherOptions,
