...
import {
  ...
  generateCreateUserDefinedTypeSQL
} from "./generator/generator_sql_mssql";
...
class OtherObjectMSSQL extends Component {
  async handleCheckboxChange(propName, event) {
    const checked = event.target.checked;
    await this.props.startTransaction(
      getHistoryContext(this.props.history, this.props.match),
      UndoRedoDef.OTHER_OBJECT_MSSQL__UPDATE_OBEJCT_OBJECT_PROPERTY_BOOLEAN
    );
    try {
  ...
  getCode() {
    ...
    switch (obj.type) {
      case OtherObjectTypes.Sequence:
        return generateCreateSequenceSQL(
          this.props.sqlSettings,
          this.props,
          obj
        );
      case OtherObjectTypes.UserDefinedType:
        return generateCreateUserDefinedTypeSQL(
          this.props.sqlSettings,
          this.props,
          obj
        );
      default:
        return obj.mssql ? obj.code : ``;
    }
  }
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(OtherObjectMSSQL)
);
