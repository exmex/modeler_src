import { QUOTATION_TYPE_DOUBLE_QUOTES } from "../../generator/sql-model-to-lines/model_to_lines";
...
export class MSSQLQuotation extends Quotation {
  ...
  leftQuote() {
    return this.sqlSettings.quotationType === QUOTATION_TYPE_DOUBLE_QUOTES
      ? `"`
      : "[";
  }
  ...
  quoteIdentifier(identifier, forceQuotation) {
    ...
    const escapedDoubleQuotesIdentifier = identifier.replace(/"/g, `""`);
    const plainIdentifier = identifier.match(/^([^\x20-\x7F]|[\w])+$/g);
    return plainIdentifier
      ? this.containsUpperChar(escapedDoubleQuotesIdentifier)
        ? `${this.leftQuote()}${escapedDoubleQuotesIdentifier}${this.rightQuote()}`
        : escapedDoubleQuotesIdentifier
      : `${this.leftQuote()}${escapedDoubleQuotesIdentifier}${this.rightQuote()}`;
  }
