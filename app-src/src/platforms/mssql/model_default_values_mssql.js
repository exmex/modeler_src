...
class ModelDefaultValuesMSSQL extends Component {
  async handleTextChange(propName, event) {
    const value = event.target.value;
    await this.props.startTransaction(
      getHistoryContext(this.props.history, this.props.match),
      UndoRedoDef.MODEL_DEFAULT_VALUES_MSSQL__UPDATE_MODEL_PROPERTY
    );
    try {
  ...
  async handleTextChangePlatform(propName, platform, event) {
    ...
    await this.props.startTransaction(
      getHistoryContext(this.props.history, this.props.match),
      UndoRedoDef.MODEL_DEFAULT_VALUES_MSSQL__UPDATE_MODEL_PROPERTY_PLATFORM
    );
    try {
      await this.props.updateModelProperty(
        this.props.match.params.mid,
  ...
  async handleCheckboxChange(propName, event) {
    ...
    await this.props.startTransaction(
      getHistoryContext(this.props.history, this.props.match),
      UndoRedoDef.MODEL_DEFAULT_VALUES_MSSQL__UPDATE_MODEL_PROPERTY_BOOLEAN
    );
    try {
      await this.props.updateModelProperty(
        this.props.match.params.mid,
      ...
      <div>
        <div className="im-properties-grid">
          ...
          <DebounceInput
            ...
            debounceTimeout={300}
            type="text"
            value={Helpers.gv(this.props.mssql.schema)}
            onChange={this.handleTextChangePlatform.bind(this, "schema", "mssql")}
          />
        </div>
      </div>
    );
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ModelDefaultValuesMSSQL)
);
