...
/* tables */
export const getSequelizeModulesDefineStatement = (
  table,
  quotations,
  generatorOptions
) => {
  ...
  if (table !== null) {
    ...
    if (table.generate === true || generatorOptions.previewObject === true) {
      ...
      if (_.size(table.indexes) > 0) {
        code += getIndexSequelizeModules(table, q);
      }
      code += "\n  }";
...
export const getIndexSequelizeModules = (table, q) => {
  var code = "";
  if (table !== null) {
    var e = 0;
    if (_.size(table.indexes) > 0) {
      ...
      for (var tableIndex of table.indexes) {
        ...
        tableIndex.fulltext && (code += "   type: fulltext, \n");
        code += "   name: " + q + tableIndex.name + q;
        tableIndex.using !== "na" &&
          tableIndex.using !== undefined &&
          (code += ", \n   method: " + tableIndex.using);
...
export const getSequelizeModulesAssociationBegin = tableName => {
  return _.upperFirst(tableName) + ".associate = function(models) {\n";
};

export const getSequelizeModulesAssociationEnd = tableAfterScript => {
  var code = "";

  code += "\n}\n";
  if (tableAfterScript && tableAfterScript !== undefined) {
    code += tableAfterScript + "\n";
  }
  code += "\n";
  return code;
};
...
/* relations */
export const getSequelizeModulesAssociation = (
  relation,
  cht,
  pt,
  quotations,
