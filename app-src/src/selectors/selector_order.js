...
const otherTypesSelector = (state) =>
  _.filter(
    state.otherObjects,
    (obj) => obj.type === OtherObjectTypes.TypeOther
  );
const userDefinedTypesSelector = (state) =>
  _.filter(
    state.otherObjects,
    (obj) => obj.type === OtherObjectTypes.UserDefinedType
  );
...
const proceduresSelector = (state) =>
  _.filter(
    state.otherObjects,
    (obj) => obj.type === OtherObjectTypes.Procedure
  );
...
function orderedItemsGroupsMSSQL(state) {
  return [
    ...getGroupItem(GROUPS.OTHER_GROUP, othersSelector(state), state.order),
    ...getGroupItem(
      GROUPS.USER_DEFINED_TYPE_GROUP,
      userDefinedTypesSelector(state),
      state.order
    ),
    ...getGroupItem(
      GROUPS.SEQUENCE_GROUP,
...
function orderComposites(orderedItemsGroups) {
  const allComposites = orderedItemsGroups
    .filter(
      (orderedItemsGroup) =>
        orderedItemsGroup.groupId === GROUPS.COMPOSITE_GROUP
    )
    .map((orderedItemsGroup) => orderedItemsGroup.item)
...
function orderedItemsByDefaultMSSQL(orderedItemsGroups) {
  orderTables(orderedItemsGroups);
  return orderedItemsGroups
    .sort((a, b) => {
      const groupsDiff = a.groupId - b.groupId;
      ...
      if (a.groupId === GROUPS.TABLE_GROUP) {
        return a.groupOrder - b.groupOrder;
      }
      return a.name.localeCompare(b.name);
    })
...
const getOrderedObjectsMSSQL = createSelector(
  orderedItemsGroupsMSSQL,
  orderedItemsByUser
);
const getOrderedObjectsDefaultMSSQL = createSelector(
  orderedItemsGroupsMSSQL,
  orderedItemsByDefaultMSSQL
);

const getOrderedObjectsForNewObjectMSSQL = createSelector(
  orderedItemsGroupsMSSQL,
  orderedItemsForNewObject
);
...
export const getOrderedObjectsReset = (modelType) => {
  if (modelType === ModelTypes.MSSQL) {
    return getOrderedObjectsDefaultMSSQL;
  } else if (modelType === ModelTypes.PG) {
...
export const getOrderedObjectsForNewObject = (modelType) => {
  if (modelType === ModelTypes.MSSQL) {
    return getOrderedObjectsForNewObjectMSSQL;
  } else if (modelType === ModelTypes.PG) {
