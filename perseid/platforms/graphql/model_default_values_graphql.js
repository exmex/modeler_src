...
class ModelDefaultValuesGraphQl extends Component {
  async handleTextChange(propName, event) {
    const value = event.target.value;
    await this.props.startTransaction(
      getHistoryContext(this.props.history, this.props.match),
      UndoRedoDef.MODEL_DEFAULT_VALUES_GRAPHQL__UPDATE_MODEL_PROPERTY
    );
    try {
  ...
  async handleCheckboxChange(propName, event) {
    ...
    await this.props.startTransaction(
      getHistoryContext(this.props.history, this.props.match),
      UndoRedoDef.MODEL_DEFAULT_VALUES_GRAPHQL__UPDATE_MODEL_PROPERTY_BOOLEAN
    );
    try {
      await this.props.updateModelProperty(
        this.props.match.params.mid,
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ModelDefaultValuesGraphQl)
);
