...
class OpenAPISchemaAssistant extends Component {
  ...
  renderFields(properties, { editable, deletable }) {
    return _.map(properties, (property, index) =>
      this.renderField(index, property.name, property.value, {
        editable,
        deletable
      })
    );
  ...
  render() {
    ...
    return (
      <div>
        {sortedSchemasForPath &&
          _.map(sortedSchemasForPath, (group) => this.renderGroup(group))}
