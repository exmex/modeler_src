...
export class MultiItemsToLines {
  ...
  isRoutineDelimiter(nestedItem) {
    return (
      nestedItem.type === TokenType.STATEMENT_DELIMITER && nestedItem.isRoutine
    );
  }
  ...
  indentAndSeparateList(nestedItem, newLine, isNotLastItem, isFirst) {
    ...
    return `${
      isFirst && this.item.type === TokenType.STATEMENT
        ? EMPTY_STRING
  ...
  indent(nestedItem, item) {
    ...
    const indent = indentationSize
      ? [...new Array(+this.sqlSettings.indentationSize).keys()]
          .map(() => indentValue)
          .join("")
      : "";
