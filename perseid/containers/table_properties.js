...
class TableProperties extends Component {
  ...
  async handleEmbeddableCheckboxChange(event) {
    ...
    await this.props.startTransaction(
      historyContext,
      UndoRedoDef.TABLE_PROPERTIES__SWITCH_EMBEDDABLE
    );
    try {
      ...
      <div>
        ...
        {this.props.match.params.id && (
          <div className="im-properties-grid">
            ...
            <div />
            {this.props.type === "MONGODB" || this.props.type === "MONGOOSE" ? (
              <CheckboxSwitch
                data-testid={TEST_ID.TABLES.EMBEDDABLE}
                label={_.upperFirst(this.props.localization.L_TABLE_EMBEDDABLE)}
                checked={Helpers.gch(
                  this.props.tables[this.props.match.params.id].embeddable
                )}
                onChange={this.handleEmbeddableCheckboxChange.bind(this)}
              />
            ) : (
              <div />
            )}
