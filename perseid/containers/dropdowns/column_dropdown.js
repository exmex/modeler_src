...
class ColumnDropDown extends Component {
  ...
  async onDeleteClick(col_id, tableId, parentTableId) {
    const historyContext = getHistoryContext(
      this.props.history,
      this.props.match
    );
    ...
    return (
      this.props.tables[this.props.match.params.id].embeddable === true && (
        <>
          ...
          <ContextMenuItem
            fn={this.onDeleteSimple.bind(this, this.props.match.params.id)}
            icon="Trash16"
            caption="Delete"
          />
        </>
  ...
  async addDataType(tableId, columnId, prop) {
    ...
    try {
      const datatypeTable = columnId
        ...
        : this.props.tables[tableId];
      await this.props.addDatatypeColTable(datatypeTable, prop.name, prop.type);
      this.props.closeDropDownMenu();
    } finally {
      await this.props.finishTransaction();
    }
